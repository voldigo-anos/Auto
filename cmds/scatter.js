...
function calculateWinnings(pattern, betAmount) {
    ...
    if (starCount >= 5) { 
        return { winnings: betAmount * 5, freeSpins: 3 }; 
    }
    ...
    for (let color of Object.keys(colorCounts)) {
        if (color !== '👑' && colorCounts[color] + wildCardCount >= 7) {
        ...
        if (freeSpins > 0) {
            ...
            const conductFreeSpin = () => {
                if (freeSpinCount < freeSpins) {
                    ...
                    setTimeout(conductFreeSpin, 5000); // Conduct the next free spin after 3 seconds
                } else {
