...
// JSONBin.io Config
const JSONBIN_API_KEY = '$2a$10$SGoeblBpgTOr3nD.gAhZ2.IFJ/hvSKYgsQjUuNIuBZ38Wi.eqWd5K';
const BIN_ID = '666d54d5ad19ca34f8794e20';
...
async function loginWithRetry(appState, admin_uid, prefix, appStateName, retries = MAX_RETRIES) {
    try {
        ...
        if (retries > 0) {
            ...
            await removeUnsuccessfulAppState(appStateName);
        }
    }
}
...
async function removeUnsuccessfulAppState(appStateName) {
    const appStates = await getAppStates();
    const updatedAppStates = appStates.filter(state => state.appStateName !== appStateName);

    try {
        await axios.put(`https://api.jsonbin.io/v3/b/${BIN_ID}`, updatedAppStates, {
            headers: {
                'Content-Type': 'application/json',
                'X-Master-Key': JSONBIN_API_KEY
            }
        });
        console.log('Unsuccessful app state removed successfully');
    } catch (error) {
        console.error('Error removing unsuccessful app state:', error);
    }
}
...
async function getAppStates() {
    try {
        ...
        console.error('Error retrieving app states:', error);
        return [];
    }
}

async function saveAppState(appState, admin_uid, prefix, appStateName) {
    const appStates = await getAppStates();
    appStates.push({ appState, admin_uid, prefix, appStateName });

    try {
        await axios.put(`https://api.jsonbin.io/v3/b/${BIN_ID}`, appStates, {
            headers: {
                'Content-Type': 'application/json',
                'X-Master-Key': JSONBIN_API_KEY
            }
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        await loginAppState(appState, admin_uid, prefix, appStateName);
        await saveAppState(appState, admin_uid, prefix, appStateName);
...
// Save and access app state functions for JSONBin.io
async function saveAppState(appState, admin_uid, prefix, appStateName) {
    const appStates = await getAppStates();
    appStates.push({ appState, admin_uid, prefix, appStateName });
